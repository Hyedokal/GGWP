...
const DOMAIN = 'GGWP-LoadBalancer-298705153.ap-northeast-2.elb.amazonaws.com';
...
export const postPersonalitiesApi = async (token: string, inputValues: InputValuesType) => {
  ...
  try {
    const response = await axios.post(`${MEMBER_API_DOMAIN}/member/personalities`, {
...
export const fetchPersonalitiesApi = async (lolNick: string | undefined, tag: string | undefined) => {
  try {
    const response = await axios.get(`${MEMBER_API_DOMAIN}/member/personalities?lolNickname=${lolNick}&tag=${tag}`);
...
export const fetchProfileImageApi = async (lolNick: string | undefined, tag: string | undefined) => {
  try {
    const response = await axios.get(`${MEMBER_API_DOMAIN}/member/profile-image?lolNickname=${lolNick}&tag=${tag}`);
...
export const uploadFileApi = async (file: File) => {
  ...
  const response = await axios.post(`${DOMAIN}/member-service/file/upload`, formData);
...
export const updateProfileImageApi = async (uploadedImageUrl: string, token: string) => {
  await axios.patch(`${MEMBER_API_DOMAIN}/member/profile-image`, {
...
export const postNewAnnouncement = async (title: string, content: string) => {
  try {
    const response = await axios.post(`${DOMAIN}/v1/announces`, {title, content});
    return response.data;
  } catch (error: unknown) {
...
export const deleteAnnouncementApi = async (announceId: number) => {
  try {
    await axios.delete(`${DOMAIN}/v1/announces/${announceId}`);
  } catch (error) {
...
export const updateAnnouncementApi = async (announcementId: number | null, title: string, content: string) => {
  ...
  try {
    await axios.put(`${DOMAIN}/v1/announces/${announcementId}`, {title, content});
  } catch (error) {
...
export const fetchAnnouncementsApi = async (page: number) => {
  try {
    const response = await axios.post(`${DOMAIN}/v1/announces/search`, {page: page});
    return response.data;
  } catch (error) {
...
export const fetchMatchesApi = async (summonerName: string | undefined, tagLine: string | undefined) => {
  try {
    const response = await axios.post<CommentMatchResponse[]>('${DOMAIN}/v1/comments/feign/matcher', {
      summonerName,
      tagLine
    });
...
export const updateUserInfoApi = async (newLolNickName: string, newTag: string, accessToken: string) => {
  return await axios.put('${DOMAIN}/member-service/v1/member/lolNicknameTag', {
...
export const fetchOpponentsInfoApi = async (ids: number[]) => {
  return await axios.post('${DOMAIN}/v1/comments/feign/match', {ids: ids});
};

// Separate function for Axios GET request to fetch match history
export const fetchMatchHistoryApi = async (accessToken: string) => {
  return await axios.get<MatchHistoryResponse>('${DOMAIN}/member-service/v1/member/match/list', {
    headers: {Authorization: `Bearer ${accessToken}`}
  });
};
...
export const deleteSquadApi = async (sid: number) => {
  try {
    await axios.delete(`${DOMAIN}/v1/squads/${sid}`);
  } catch (error) {
...
export const fetchSquadsApi = async () => {
  try {
    const response = await axios.post('${DOMAIN}/v1/squads/search', {
      outdated: false,
      page: 0,
      size: 15
    });
...
export const updateSquadApi = async (sid: number, editedPost: BoardListResponseDto) => {
  try {
    await axios.put(`${DOMAIN}/v1/squads/${sid}`, editedPost);
  } catch (error) {
...
export const approveCommentApi = async (cid: number) => {
  try {
    const response = await axios.put(`${DOMAIN}/v1/squads/comment/approve/${cid}`);
...
export const postCommentApi = async (sId: number, wontPos: string, qType: string, useMic: boolean, summonerName: string, tagLine: string, memo: string) => {
  try {
    const response = await axios.post('${DOMAIN}/v1/comments', {
      sId,
      wontPos,
      qType,
      useMic,
...
export const fetchCommentsApi = async (sId: number, page: number) => {
  try {
    const response = await axios.get(`${DOMAIN}/v1/squads/${sId}?page=${page}`);
...
export const createNoticeApi = async (cid: number) => {
  try {
    const response = await axios.post('${DOMAIN}/notice-service/v1/notice/create', {cid});
